...
using DB = std::unordered_map<std::string, Record, StringHash, std::equal_to<>>;
...
DB process_input(std::span<const char> data) {
    ...
    while (iter != data.end()) {
        ...
        auto station = std::string(iter, semi_col);
        iter = semi_col + 1;
        ...
        float fp_value;
        std::from_chars(std::to_address(iter), std::to_address(new_line), fp_value);
        iter = new_line + 1;
